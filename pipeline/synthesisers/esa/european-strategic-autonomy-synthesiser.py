#!/usr/bin/env python3
"""
European Strategic Autonomy Monitor Synthesiser
Monitor slug : european-strategic-autonomy
Model       : sonar-pro (no web search — reasons over supplied docs)
Output      : pipeline/monitors/european-strategic-autonomy/synthesised/synthesis-latest.json
"""
...
PROMPT_FILE    = pathlib.Path(__file__).with_name("european-strategic-autonomy-synthesiser-api-prompt.txt")
RESPONSE_SCHEMA = pathlib.Path(__file__).with_name("esa-response-schema.json")
METHODOLOGY = REPO_ROOT / "docs" / "methodology" / "european-strategic-autonomy-full.md"
IDENTITY    = REPO_ROOT / "docs" / "identity" / "esa-identity.md"
ADDENDUM    = REPO_ROOT / "docs" / "methodology" / "european-strategic-autonomy-addendum.md"
...
def load_json(path):
    p = pathlib.Path(path)
    if not p.exists():
        print(f"[ESA] WARNING: {p} not found — skipping")
        return {}
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except json.JSONDecodeError as e:
        print(f"[ESA] WARNING: Could not parse {p}: {e}")
        return {}
...
if not prompt_text:
    sys.exit(f"[ESA] ERROR: prompt file not found at {PROMPT_FILE}")

system_msg = (
    "You are a systematic intelligence synthesiser for the European Strategic Autonomy Monitor "
    "at asym-intel.info. "
...
if len(user_msg) > MAX_CONTEXT:
    print(f"[ESA] Context truncated: {len(user_msg)} → {MAX_CONTEXT} chars")
...
if response_schema:
    ...
    print("[ESA] Using structured output (response_format)")

print(f"[ESA] Calling {MODEL} …")
resp = requests.post(
    API_URL,
...
try:
    synthesis, was_repaired = parse_llm_json(raw, "ESA")
    if was_repaired:
        print("[ESA] JSON repaired successfully")
        log_incident(monitor="european-strategic-autonomy", stage="synthesiser", incident_type="json_repaired",
                     severity="info", detail="JSON required repair before parsing")
except json.JSONDecodeError as e:
    print(f"[ESA] JSON parse error: {e}. Writing fallback stub.")
    log_incident(monitor="european-strategic-autonomy", stage="synthesiser", incident_type="json_parse_error",
                 severity="error", detail=f"JSON parse error: {e}. Fallback stub written.",
                 raw_snippet=raw[:500] if raw else "")
    synthesis = {
        "_meta": {
            "schema_version": "european-strategic-autonomy-synthesis-v1.0",
...
if _TIM_AVAILABLE:
    ...
    try:
        _tim_result = _tim_monitor(
            ...
            _usage,
            "pipeline/monitors/european-strategic-autonomy/ops/ops-metrics.json",
        )
...
print(f"[ESA] Written → {OUT_DATED}")
print(f"[ESA] null_signal_week = {null_flag}")
