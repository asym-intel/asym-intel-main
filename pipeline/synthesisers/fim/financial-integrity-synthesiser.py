#!/usr/bin/env python3
"""
Financial Integrity Monitor Synthesiser
Monitor slug : financial-integrity
Model       : sonar-pro (no web search — reasons over supplied docs)
Output      : pipeline/monitors/financial-integrity/synthesised/synthesis-latest.json
"""
...
MONITOR_DIR = REPO_ROOT / "pipeline" / "monitors" / "financial-integrity"
SYNTH_DIR   = MONITOR_DIR / "synthesised"
PROMPT_FILE    = pathlib.Path(__file__).with_name("financial-integrity-synthesiser-api-prompt.txt")
RESPONSE_SCHEMA = pathlib.Path(__file__).with_name("fim-response-schema.json")
METHODOLOGY    = REPO_ROOT / "docs" / "methodology" / "financial-integrity-full.md"
IDENTITY       = REPO_ROOT / "docs" / "identity" / "fim-identity.md"
ADDENDUM       = REPO_ROOT / "docs" / "methodology" / "financial-integrity-addendum.md"
...
if OUT_DATED.exists():
    print(f"[FIM] GUARD: synthesiser already ran today ({TODAY_STR}). Exiting.")
    sys.exit(0)

def load_json(path):
    p = pathlib.Path(path)
    if not p.exists():
        print(f"[FIM] WARNING: {p} not found — skipping")
        return {}
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except json.JSONDecodeError as e:
        print(f"[FIM] WARNING: Could not parse {p}: {e}")
        return {}
...
DATA_DIR          = REPO_ROOT / "docs" / "monitors" / "financial-integrity" / "data"
...
if not prompt_text:
    sys.exit(f"[FIM] ERROR: prompt file not found at {PROMPT_FILE}")

system_msg = (
    "You are a systematic intelligence synthesiser for the Financial Integrity Monitor "
    "at asym-intel.info. "
...
# ── Extract key sections from persistent-state ────────────────────────────
ps_extract = {}
for key in ("jurisdiction_baselines", "scheme_inventory", "enforcement_gap_log",
            "regulatory_horizon", "ctf_cpf_flags", "cross_monitor_flags"):
...
if ps_extract:
    parts.append("## PERSISTENT STATE (jurisdiction baselines + scheme inventory)\n\n"
...
if len(user_msg) > MAX_CONTEXT:
    print(f"[FIM] Context truncated: {len(user_msg)} → {MAX_CONTEXT} chars")
...
if response_schema:
    ...
    print("[FIM] Using structured output (response_format)")

print(f"[FIM] Calling {MODEL} …")
resp = requests.post(
    API_URL,
...
if resp.status_code == 429:
    print(f"[FIM] 429 rate limit — waiting 60s")
    time.sleep(60)
...
try:
    synthesis, was_repaired = parse_llm_json(raw, "FIM")
    if was_repaired:
        print("[FIM] JSON repaired successfully")
        log_incident(monitor="financial-integrity", stage="synthesiser", incident_type="json_repaired",
                     severity="info", detail="JSON required repair before parsing")
except json.JSONDecodeError as e:
    print(f"[FIM] JSON parse error: {e}. Writing fallback stub.")
    log_incident(monitor="financial-integrity", stage="synthesiser", incident_type="json_parse_error",
                 severity="error", detail=f"JSON parse error: {e}. Fallback stub written.",
                 raw_snippet=raw[:500] if raw else "")
    synthesis = {
        "_meta": {
            "schema_version": "fim-synthesis-v1.0",
            "research_model": MODEL,
            "monitor_slug": "financial-integrity",
...
if _TIM_AVAILABLE:
    ...
    try:
        _tim_result = _tim_monitor(
            "financial-integrity",
            _usage,
            "pipeline/monitors/financial-integrity/ops/ops-metrics.json",
        )
...
print(f"[FIM] Written → {OUT_DATED}")
print(f"[FIM] null_signal_week = {null_flag}")
