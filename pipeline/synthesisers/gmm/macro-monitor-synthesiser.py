...
# ── Persistent-state inventory for prompt injection (Sprint-7 P2b) ────────────
...
stored_state_inventory = render_stored_state_inventory(
    ...
    persistent_state_path=REPO_ROOT / "docs" / "monitors" / "macro-monitor" / "data" / "persistent-state.json",
...
try:
    ...
    if was_repaired:
        ...
        log_incident(monitor="macro-monitor", stage="synthesiser", incident_type="json_repaired",
                     severity="info", detail="JSON required repair before parsing")
except json.JSONDecodeError as e:
    print(f"[GMM] JSON parse error: {e}. Writing fallback stub.")
    log_incident(monitor="macro-monitor", stage="synthesiser", incident_type="json_parse_error",
