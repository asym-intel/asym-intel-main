...
MONITOR_DIR = REPO_ROOT / "pipeline" / "monitors" / "ai-governance"
SYNTH_DIR   = MONITOR_DIR / "synthesised"
PROMPT_FILE    = pathlib.Path(__file__).with_name("ai-governance-synthesiser-api-prompt.txt")
RESPONSE_SCHEMA = pathlib.Path(__file__).with_name("agm-response-schema.json")
METHODOLOGY = REPO_ROOT / "docs" / "methodology" / "ai-governance-full.md"
IDENTITY    = REPO_ROOT / "docs" / "identity" / "agm-identity.md"
ADDENDUM    = REPO_ROOT / "docs" / "methodology" / "ai-governance-addendum.md"
...
if OUT_DATED.exists():
    print(f"[AGM] GUARD: synthesiser already ran today ({TODAY_STR}). Exiting.")
    sys.exit(0)

def load_json(path):
    p = pathlib.Path(path)
    if not p.exists():
        print(f"[AGM] WARNING: {p} not found — skipping")
        return {}
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except json.JSONDecodeError as e:
        print(f"[AGM] WARNING: Could not parse {p}: {e}")
        return {}
...
if response_schema:
    ...
    print("[AGM] Using structured output (response_format)")

print(f"[AGM] Calling {MODEL} …")
resp = requests.post(
    API_URL,
...
try:
    synthesis, was_repaired = parse_llm_json(raw, "AGM")
    if was_repaired:
        print("[AGM] JSON repaired successfully")
        log_incident(monitor="ai-governance", stage="synthesiser", incident_type="json_repaired",
                     severity="info", detail="JSON required repair before parsing")
except json.JSONDecodeError as e:
    print(f"[AGM] JSON parse error: {e}. Writing fallback stub.")
    log_incident(monitor="ai-governance", stage="synthesiser", incident_type="json_parse_error",
                 severity="error", detail=f"JSON parse error: {e}. Fallback stub written.",
                 raw_snippet=raw[:500] if raw else "")
    synthesis = {
        "_meta": {
            "schema_version": "ai-governance-synthesis-v1.0",
...
if _TIM_AVAILABLE:
    ...
    try:
        _tim_result = _tim_monitor(
            "ai-governance",
            _usage,
            "pipeline/monitors/ai-governance/ops/ops-metrics.json",
        )
...
print(f"[AGM] Written → {OUT_DATED}")
print(f"[AGM] null_signal_week = {null_flag}")
