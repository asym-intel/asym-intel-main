...
Row count: **29** (21 prompts + 7 schemas + 1 methodology-guardrails). FCW synth-schema promoted from canonical-only to byte-identical as part of v1.3 FCW canonical sprint (AD-22j) — response-schema authored byte-exact on main at the same time. Unchanged at v1.5 — SCEM Leg A adds only canonical-only rows; byte-identical byte-lift on SCEM synth script and synth-prompt header follows in Leg B1 under AD-22e.
