#!/usr/bin/env python3
"""
FCW Chatter — Daily Signal
Calls Perplexity sonar for top 10 raw FIMI signal items.
Writes to static/monitors/fimi-cognitive-warfare/data/chatter-latest.json (public)
       and static/monitors/fimi-cognitive-warfare/data/chatter-YYYY-MM-DD.json
"""
...
# Load prompt
PROMPT_FILE = pathlib.Path(f"pipeline/monitors/{SLUG}/fcw-chatter-api-prompt.txt")
...
# Call Perplexity API
print(f"Calling sonar for FCW Chatter ({TODAY})...")
...
for i, item in enumerate(items):
    if not item.get("item_id"):
        item["item_id"] = f"fcw-chatter-{TODAY}-{i+1:03d}"
