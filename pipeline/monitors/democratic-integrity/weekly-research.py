#!/usr/bin/env python3
"""
WDM Weekly Research — GitHub Actions script
Calls Perplexity sonar-pro API with the WDM weekly research prompt,
validates schema, and writes output to pipeline/monitors/democratic-integrity/weekly/

Runs: Sunday 18:00 UTC (12 hours before Monday WDM Analyst at 06:00 UTC)
...
OUT_DIR     = pathlib.Path("pipeline/monitors/democratic-integrity/weekly")
...
PROMPT_FILE = pathlib.Path(
    "pipeline/monitors/democratic-integrity/wdm-weekly-research-api-prompt.txt"
)
    ...
    try:
        ...
        if _sec_start > -1:
            ...
            prompt += (
                ...
                + str(_calib_year)
                + " — apply these country-specific LDI anchors and ERT flags:\n"
                + _inject
            )
...
try:
    ...
    log_incident(monitor="democratic-integrity", stage="weekly-research", incident_type="json_parse_error",
...
REQUIRED_LEAD = ["headline", "country", "confidence_preliminary", "source_url"]
...
if meta.get("monitor_slug") != "democratic-integrity":
    errors.append(f"monitor_slug '{meta.get('monitor_slug')}' — expected 'democratic-integrity'")
...
if not data.get("erosion_developments"):
    print("INFO: erosion_developments array is empty — quiet week is valid, but unusual for WDM")
...
# Source corroboration check for High/Confirmed developments
...
# Rationale: sonar-pro systematically returns single-source findings.
# Blocking valid data at the pipeline stage prevents the analyst from seeing it.
# The analyst cron applies methodology and assigns final confidence.
# See methodology doc §Source Corroboration for the full rule.
for i, dev in enumerate(data.get("erosion_developments", [])):
    conf = dev.get("attribution_confidence_preliminary", "")
    if conf in ("Confirmed", "High"):
        sources = dev.get("research_traceback", {}).get("sources_cited", [])
        if len(sources) == 0:
            errors.append(
                f"erosion_developments[{i}] ({dev.get('development_id','?')}) is {conf} "
                f"but has 0 sources — cannot accept without any evidence"
            )
        elif len(sources) == 1:
            dev["confidence_original"] = conf
            dev["attribution_confidence_preliminary"] = "Moderate"
            warnings.append(
                f"erosion_developments[{i}] ({dev.get('development_id','?')}) downgraded "
                f"{conf} → Moderate (single source — analyst may upgrade if authoritative)"
            )

if errors:
    log_incident(monitor="democratic-integrity", stage="weekly-research", incident_type="schema_violation",
    ...
    print(f"erosion_developments count: {len(data.get('erosion_developments',[]))}")
    OUT_DIR.mkdir(parents=True, exist_ok=True)
...
data["_meta"]["monitor_slug"] = "democratic-integrity"
data["_meta"]["development_count"] = len(data.get("erosion_developments", []))
data["_meta"]["electoral_count"]   = len(data.get("electoral_watch", []))
data["_meta"]["institutional_count"] = len(data.get("institutional_events", []))
...
print(f"   Erosion developments: {len(data.get('erosion_developments', []))}")
print(f"   Electoral watch: {len(data.get('electoral_watch', []))}")
print(f"   Institutional events: {len(data.get('institutional_events', []))}")
print(f"   Autocratic export: {len(data.get('autocratic_export', []))}")
