#!/usr/bin/env python3
"""
GMM Chatter — Daily Signal
Calls Perplexity sonar for top 10 raw FIMI signal items.
Writes to static/monitors/macro-monitor/data/chatter-latest.json (public)
       and static/monitors/macro-monitor/data/chatter-YYYY-MM-DD.json
"""
...
# Load prompt
PROMPT_FILE = pathlib.Path(f"pipeline/monitors/{SLUG}/gmm-chatter-api-prompt.txt")
...
# Call Perplexity API
print(f"Calling sonar for GMM Chatter ({TODAY})...")
...
for i, item in enumerate(items):
    if not item.get("item_id"):
        item["item_id"] = f"gmm-chatter-{TODAY}-{i+1:03d}"
