#!/usr/bin/env python3
"""
GMM Collector — GitHub Actions script
Calls Perplexity API with the GMM Collector prompt,
...
OUT_DIR   = pathlib.Path("pipeline/monitors/macro-monitor/daily")
...
if raw_content is None:
    log_incident(monitor="macro-monitor", stage="collector", incident_type="api_failure",
...
try:
    ...
    log_incident(monitor="macro-monitor", stage="collector", incident_type="json_parse_error",
...
# GMM: also require signal_type (SA/CC/TS)
GMM_REQUIRED_FINDING_EXTRA = ["signal_type"]
...
if meta.get("monitor_slug") != "macro-monitor":
    errors.append(f"monitor_slug is '{meta.get('monitor_slug')}' — expected 'macro-monitor'")
...
if errors:
    log_incident(monitor="macro-monitor", stage="collector", incident_type="schema_violation",
