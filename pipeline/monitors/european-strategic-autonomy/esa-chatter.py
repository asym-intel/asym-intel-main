...
# Load prompt
PROMPT_FILE = pathlib.Path(f"pipeline/monitors/{SLUG}/esa-chatter-api-prompt.txt")
...
# Call Perplexity API
print(f"Calling sonar for ESA Chatter ({TODAY})...")
...
for i, item in enumerate(items):
    if not item.get("item_id"):
        item["item_id"] = f"esa-chatter-{TODAY}-{i+1:03d}"
