#!/usr/bin/env python3
"""
SCEM Collector — GitHub Actions script
Calls Perplexity API with the SCEM Collector prompt,
...
PROMPT_FILE = pathlib.Path(
    "pipeline/monitors/conflict-escalation/scem-collector-api-prompt.txt"
)
...
if raw_content is None:
    log_incident(monitor="conflict-escalation", stage="collector", incident_type="api_failure",
...
try:
    ...
    log_incident(monitor="conflict-escalation", stage="collector", incident_type="json_parse_error",
...
if meta.get("monitor_slug") != "conflict-escalation":
    errors.append(f"monitor_slug is '{meta.get('monitor_slug')}' — expected 'conflict-escalation'")
...
if errors:
    log_incident(monitor="conflict-escalation", stage="collector", incident_type="schema_violation",
...
if _TIM_AVAILABLE:
    ...
    try:
        _tim_result = _tim_monitor(
            "conflict-escalation",
            _usage,
            "pipeline/monitors/conflict-escalation/ops/ops-metrics.json",
        )
