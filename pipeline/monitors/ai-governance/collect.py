#!/usr/bin/env python3
"""
AGM Collector — GitHub Actions script
Calls Perplexity API with the AGM Collector prompt,
...
OUT_DIR   = pathlib.Path("pipeline/monitors/ai-governance/daily")
...
PROMPT_FILE = pathlib.Path(
    "pipeline/monitors/ai-governance/agm-collector-api-prompt.txt"
)
...
_calib_path = f"methodology/ai-governance-eeas-{_calib_year}.md"
...
if raw_content is None:
    log_incident(monitor="ai-governance", stage="collector", incident_type="api_failure",
...
try:
    ...
    log_incident(monitor="ai-governance", stage="collector", incident_type="json_parse_error",
...
if meta.get("monitor_slug") != "ai-governance":
    errors.append(f"monitor_slug is '{meta.get('monitor_slug')}' — expected 'ai-governance'")
...
if errors:
    log_incident(monitor="ai-governance", stage="collector", incident_type="schema_violation",
...
if _TIM_AVAILABLE:
    ...
    _usage["citations"] = len(citations)
    _usage["unique_domains"] = len(set(
        c.split("/")[2] if "/" in c else c
        for c in citations
    ))
    try:
