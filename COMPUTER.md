...
## Session-End Checklist (run before closing any session where changes were made)

Every session that modifies code, data, prompts, or methodology must complete
these before ending. HANDOFF.md is auto-generated by Housekeeping every Monday --
but if significant changes were made mid-week, update it manually.
...
**Mandatory before ending if true:**
- [ ] New cron created -> add to cron table in COMPUTER.md
- [ ] New GitHub Actions workflow -> add to Active GitHub Actions table
- [ ] Architecture decision made -> add to docs/audits/ or site-decisions.json
- [ ] Methodology gap identified and fixed -> logged in notes-for-computer.md
- [ ] Annual calibration file added -> verify {slug}-{index}-{YEAR}.md naming convention

**HANDOFF.md is auto-generated by Housekeeping (Monday 08:00 UTC).**
If ending a mid-week session with significant changes: update HANDOFF.md manually.
If ending a session after Monday: HANDOFF.md is already current from that morning.
