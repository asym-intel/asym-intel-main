...
**Scope: live OSINT collection only.** sonar/sonar-pro have live web search; Computer does not.
This is the only genuine capability gap that justifies the pipeline split.

### What to outsource via API (GitHub Actions pipeline)

| Task | Model | Why |
|---|---|---|
| Daily OSINT collection | sonar | Live web search — Computer cannot do this |
| Weekly deep research | sonar-pro | Live web search + depth |
| Reasoner synthesis | sonar-deep-research | Reasoning over pipeline documents |

### What stays in Computer sessions — do NOT outsource

| Task | Why |
|---|---|
| HTML/CSS component drafts and variants | sonar-pro averages toward the middle — variants come out near-identical, defeating design review. Computer writes, screenshots with Playwright, and iterates in one session at lower net cost. |
| Monitor Overview page templates | Same as above — visual hierarchy requires a feedback loop |
| Python script drafts | Computer writes with full repo context; sonar-pro has no context |
| Prompt drafts | Computer writes against spec; sonar-pro has no access to methodology files |
| Visual review and variant selection | Requires judgement + visual confirmation |
| Promoting drafts to production | Requires architecture knowledge and repo writes |
| Debugging broken output | Requires repo context |
| Governance doc updates | Requires session context |

**Rationale (7 April 2026):** sonar-pro HTML/CSS generation was reviewed against the Sprint 1
components draft. Output was structurally correct but variants were near-identical — insufficient
differentiation for design decisions. No visual feedback loop means errors propagate to review.
Computer does the same work natively with Playwright screenshots, iterates to genuinely distinct
variants, and costs less overall. `tools/generate-sprint1-batch.py` is retired — Sprint 1
components will be built directly in-session using the draft's token system as a starting point.
