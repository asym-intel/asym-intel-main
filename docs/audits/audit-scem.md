...
|---|---|---|---|---|
| **1** | ~~**Fix dashboard bar chart: add baseline marker to I1–I6 bars**~~ ✅ 2026-04-01 | Critical — the core concept (deviation, not level) is not communicated; every user who reads the dashboard misunderstands the analytical framework | Low — CSS/SVG change to existing bar chart component | Frontend |
| **2** | ~~**Fix report.html rendering bug: Roster Watch and Cross-Monitor sections empty**~~ ✅ 2026-04-01 Confirmed rendering correctly — data was sparse in prior issues, not a template bug | High — two complete JSON sections are silently dropped from the canonical intelligence report | Low | Backend/Template |
...
| **4** | ~~**Add CONTESTED baseline disclaimer to Global Escalation Index**~~ ✅ 2026-04-01 Index now uses deviation magnitude not band; CONTESTED note shown when baselines unset | High — index read LOW during active US-Iran war with Hormuz closed | Medium | Data + Frontend |
