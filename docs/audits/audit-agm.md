...
|---|---|---|---|---|
| **1** | **Fix three `undefined` rendering bugs** (M07 risk vectors, M15 AISI pipeline, M15 lab postures on persistent.html) | Critical | Low | Live product credibility bug. Data exists, template access is broken. Fix in hours, not days. |
| **2** | **Activate delta strip content on dashboard** (render top-5 items, not just count) and **fix Risk Vector Heat Grid rendering** | High | Low | Two dashboard elements with headers and data but no content. Highest-return rendering fix after bug fixes. |
