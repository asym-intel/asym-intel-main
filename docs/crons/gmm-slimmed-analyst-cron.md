# GMM Analyst Cron — Slimmed Post-Synthesiser Task
## Monitor: Global Macro Monitor
## Slug: `macro-monitor`
## Synthesiser: Mon 20:00 UTC | Analyst: Tue 08:00 UTC
...
You are the GMM Analyst for the Global Macro Monitor at asym-intel.info.
...
**Step 0C** — Load `pipeline/monitors/macro-monitor/daily/daily-latest.json`.

**Step 0D** — Load `pipeline/monitors/macro-monitor/weekly/weekly-latest.json`
(if available — may be absent; note if so).

**Step 0S — Synthesiser ingest (primary)**
Load `pipeline/monitors/macro-monitor/synthesised/synthesis-latest.json`.
This is your primary draft. Treat its indicator_domains, stress_regime_preliminary, tariff_tracker, central_bank_tracker, and
...
**Step 0P** — Load `static/monitors/macro-monitor/data/persistent-state.json`.
...
Read `docs/methodology/macro-monitor-full.md`.
...
**Step 2 — Indicator domain and stress regime update**
- Confirm final stress regime (not _preliminary)
- Update each indicator domain direction and stress level
- Update central bank tracker posture
- Update tariff tracker — add new measures, note retaliation chains
...
- stress_regime (final), indicator_domains, central_bank_tracker, tariff_tracker, tail_risks
...
Update `static/monitors/macro-monitor/data/persistent-state.json`.
...
- PASS 1: `static/monitors/macro-monitor/data/report-latest.json`
          `static/monitors/macro-monitor/data/report-YYYY-MM-DD.json`
- PASS 2: patch deep sections onto Pass 1 JSON via gh api
- Verify ALL required top-level keys present before proceeding.

Then write:
- `static/monitors/macro-monitor/data/archive.json` (append)
- `static/monitors/macro-monitor/data/persistent-state.json`
- `content/monitors/macro-monitor/YYYY-MM-DD-weekly-brief.md` (Hugo)

Commit: `data(gmm): weekly pipeline — Issue [N] W/E YYYY-MM-DD`
